...
class Tomcat7 implements CommonTomcat<Host, JarScanner, LifecycleException> {
    ...
    Tomcat7(Tomcat tomcat) {
        this.tomcat = tomcat;
    }

    @Override
    public CommonEngine getEngine() {
        return new Tomcat7Engine(tomcat.getEngine());
    }
    ...
    public CommonContext<JarScanner> addWebApp(Host host, String contextPath, String docBase) {
        return new Tomcat7Context(tomcat.addWebapp(host, contextPath, docBase));
    }

    @Override
    public CommonConnector getConnector() {
        return new Tomcat7Connector(tomcat.getConnector());
    }
    ...
    public CommonWrapper addServlet(String contextPath, String name, Servlet servlet) {
        return new Tomcat7Wrapper(tomcat.addServlet(contextPath, name, servlet));
    }
